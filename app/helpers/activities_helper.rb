module ActivitiesHelper
  ...
  def feed_message(activity)
    ...
    when "Exchange"
      ...
      %(#{person_link(person)} #{t('earned')} #{nice_decimal(exchange.amount)} #{h exchange.group.unit.pluralize} #{t('for')}
  ...
  def minifeed_message(activity)
    ...
    when "Exchange"
      exchange = activity.item
      %(#{h person.display_name} #{t('earned')} #{ nice_decimal(exchange.amount)} #{h exchange.group.unit.pluralize} #{t('for')}
      #{metadata_link(exchange.metadata)} #{t('in')} #{group_link(exchange.group)}.).html_safe
    else
      raise "Invalid activity type #{activity_type(activity).inspect}"
    end
